...
class AlbumCollectionApp {
    ...
    // Manual image loading triggered by user button click
    async manuallyLoadArtistImages() {
        try {
            const loadButton = document.getElementById('load-artist-images');
            if (!loadButton) return;

            // Check if ImageService is available
            if (!window.ImageService) {
                alert('Image service is not available. Please refresh the page.');
                return;
            }
            ...
            if (allArtists.length === 0) {
                alert('No artists available to load images for.');
                return;
            }
            ...
            if (artistsNeedingImages.length === 0) {
                alert('All artists already have images loaded!');
                return;
            }

            console.log(`🖼️ Manual image loading for ${artistsNeedingImages.length} artists`);

            // Use batch processing with progress updates
            let processedCount = 0;
            const results = await imageService.batchProcessArtists(artistsNeedingImages, (artist, imageUrl) => {
                processedCount++;
                loadButton.textContent = `🖼️ Loading ${processedCount}/${artistsNeedingImages.length}...`;
                
                // Update UI immediately when image is found
                if (imageUrl) {
                    this.updateArtistCardImage(artist, imageUrl);
                }
            });
            ...
            const message = successCount > 0 
                ? `✅ Successfully loaded ${successCount} artist images!`
                : '⚠️ No new images were found for your artists.';
            
            alert(message);
            console.log(`🖼️ Manual loading completed: ${successCount}/${artistsNeedingImages.length} images loaded`);

        } catch (error) {
            console.error('❌ Error in manual image loading:', error);
            alert('❌ An error occurred while loading images. Check the console for details.');
        } finally {
            // Reset button state
            const loadButton = document.getElementById('load-artist-images');
            if (loadButton) {
                ...
                loadButton.textContent = '🖼️ Load Images';
            }
        }
    }
    ...
    showArtistAlbums(artist) {
        ...
        // Include album count in title and optionally artist image
        const artistImage = artist.image ? `<img src="${artist.image}" alt="${artist.name}" class="modal-title-image">` : '';
        const titleWithImage = `${artistImage}${artist.name} - Albums (${albumCount})`;
                                ...
                                if (roleName && typeof roleName === 'string') {
                                    ...
                                    if (cleanRoleName) {
                                        ...
                                        if (roleMap.has(roleKey)) {
                                            ...
                                            if (!existingRole.artists.some(artist => artist.name === artistName)) {
                                                existingRole.artists.push({
                                                    name: artistName,
                                                    id: credit.id || null
                                                });
                                                console.log(`       Added artist "${artistName}" to existing role "${cleanRoleName}"`);
                                            }
                                            
                                            // Add album if not already included  
                                            if (!existingRole.albums.some(a => a.id === album.id)) {
                                                existingRole.albums.push({
                                                    id: album.id,
                                                    title: album.title,
                                                    year: album.year,
                                                    image: album.images && album.images[0] ? album.images[0].uri : null,
                                            ...
                                            const newRole = {
                                                id: `role-${roleKey.replace(/\s+/g, '-').replace(/[^\w-]/g, '')}`,
                                                name: cleanRoleName,
                                                frequency: 1,
                                                artists: [{
                                                    name: artistName,
                                            ...
                                            roleMap.set(roleKey, newRole);
                                            console.log(`       Created new role "${cleanRoleName}" with artist "${artistName}"`);
                                        }
                                    }
                                } else {
                                    console.warn(`     Skipping invalid role:`, roleName);
                                }
                            });
                        });
                    }
                } catch (albumError) {
                    console.warn(`⚠️ Error processing roles for album "${album.title}":`, albumError);
                }
            });
        } catch (error) {
