...
class AlbumCollectionApp {
    ...
    // Enhanced album loading with single request
    ...
    async loadAlbumsWithProgress() {
        ...
        this.updateLoadingProgress('📊 Calculating collection size...', 'Checking album count...', 30);
        ...
        if (countError) {
            console.warn('⚠️ Could not get exact count, proceeding anyway');
        }



        console.log(`📊 Total albums: ${totalCount || 'unknown'}, Loading in single request...`);

        // Update progress before loading
        this.updateLoadingProgress(
            '🚀 Loading all albums...',
            `Fetching ${totalCount || 'all'} albums from database...`,
            40
        );

        // Load all albums in single request via the Supabase service
        const allAlbums = await this.dataService.getAllAlbums();
            ...
            <div class="artist-roles-section">
                <div class="artist-role-tabs">
                    ...
                    <button class="role-tab-btn ${collaboratorsTabActive ? 'active' : ''}"
                        ...
                        Collaborators (${artistCollaborators.length})
                    </button>
                </div>
                <div class="artist-role-content">
                    ...
                    <div id="collaborators-${artistId}" class="role-tab-content ${collaboratorsTabActive ? 'active' : ''}">
                        ${collaboratorsHtml}
                    </div>
                </div>
            </div>
    ...
    // Get all unique collaborators from albums where artist performed
    getArtistCollaborators(artistName) {
        const collaboratorFrequency = new Map();
        
        // Go through all albums and collect other artists where this artist performed
        this.collection.albums.forEach(album => {
            // Check if target artist is in this album's credits
            const targetArtistIsInAlbum = album.credits && album.credits.some(credit => 
                credit.name === artistName
            );
            
            if (targetArtistIsInAlbum && album.credits) {
                // Find all other artists in this album (excluding the target artist)
                const otherArtists = album.credits
                    .map(credit => credit.name)
                    .filter(name => name !== artistName) // Exclude the target artist
                    .filter((name, index, array) => array.indexOf(name) === index); // Remove duplicates within this album
                
                // Count frequency of each collaborator
                otherArtists.forEach(collaboratorName => {
                    if (collaboratorName && collaboratorName.trim()) {
                        const cleanName = collaboratorName.trim();
                        collaboratorFrequency.set(cleanName, (collaboratorFrequency.get(cleanName) || 0) + 1);
                    }
                });
            }
        });
        ...
        const sortedCollaborators = Array.from(collaboratorFrequency.entries())
            .sort((a, b) => b[1] - a[1]) // Sort by frequency descending
    ...
    switchArtistRoleTab(artistId, tabType) {
        ...
        const collaboratorsContent = document.getElementById(`collaborators-${artistId}`);

        // Hide all content first
        musicalContent.classList.remove('active');
        technicalContent.classList.remove('active');
        genresContent.classList.remove('active');
        collaboratorsContent.classList.remove('active');
        ...
        } else if (tabType === 'collaborators') {
            collaboratorsContent.classList.add('active');
        }
    }
    ...
    filterAlbumsByCollaborators(artistName) {
        const albumsGrid = document.getElementById('artist-albums-grid');
        if (!albumsGrid) {
            console.error('❌ Albums grid not found');
            return;
        ...
        console.log(`🎯 Collaboration filter result: ${filteredAlbums.length} albums visible out of ${allAlbums.length} total`);
    ...
    // Multi-select state management for artist modal filters
    initializeArtistModalFilters(artistName) {
        if (!this.artistModalFilters) {
            this.artistModalFilters = new Map();
        }
        
        if (!this.artistModalFilters.has(artistName)) {
    ...
    // Toggle role selection in artist modal (MULTI-SELECT)
    toggleArtistModalRole(artistName, role, element) {
        this.initializeArtistModalFilters(artistName);
        const filters = this.artistModalFilters.get(artistName);
        
        // Toggle role selection
        if (filters.selectedRoles.has(role)) {
            filters.selectedRoles.delete(role);
            element.classList.remove('active-filter');
            console.log(`🎭 Deselected role: ${role}`);
        } else {
            filters.selectedRoles.add(role);
            element.classList.add('active-filter');
    ...
    // Toggle genre selection in artist modal (MULTI-SELECT)
    toggleArtistModalGenre(artistName, genre, element) {
        this.initializeArtistModalFilters(artistName);
        const filters = this.artistModalFilters.get(artistName);
        
        // Toggle genre selection
        if (filters.selectedGenres.has(genre)) {
    ...
    // Apply multi-select filtering to artist modal albums
    applyArtistModalFilters(artistName) {
        const albumsGrid = document.getElementById('artist-albums-grid');
        if (!albumsGrid) return;
        ...
        // Apply role filtering (AND logic - album must have ALL selected roles)
        if (filters.selectedRoles.size > 0) {
            filteredAlbums = filteredAlbums.filter(album => {
                if (!album.credits) return false;
                
                return Array.from(filters.selectedRoles).every(selectedRole => {
                    return album.credits.some(credit => 
                        credit.name === artistName && 
                        credit.role && 
                        this.smartSplitRoles(credit.role).includes(selectedRole)
                    );
                });
            });
        }

        // Apply genre filtering (AND logic - album must have ALL selected genres)
        if (filters.selectedGenres.size > 0) {
            filteredAlbums = filteredAlbums.filter(album => {
                const albumGenres = [];
                if (album.genres) albumGenres.push(...album.genres);
                if (album.styles) albumGenres.push(...album.styles);
                
                return Array.from(filters.selectedGenres).every(selectedGenre => 
                    albumGenres.includes(selectedGenre)
                );
            });
        }
    ...
    updateArtistModalAlbumsDisplay(filteredAlbums, totalCount) {
        const albumsGrid = document.getElementById('artist-albums-grid');
        if (!albumsGrid) return;

        // Update album count
        const albumCountElement = albumsGrid.parentElement.querySelector('.album-count');
        if (albumCountElement) {
            albumCountElement.textContent = `${filteredAlbums.length} album${filteredAlbums.length !== 1 ? 's' : ''}`;
        }

        // Generate new album cards
        const albumCardsContainer = document.createElement('div');
        filteredAlbums.forEach(album => {
            ...
            const albumCard = new AlbumCard(album);
            albumCardsContainer.appendChild(albumCard.render());
        });

        // Replace current albums
        albumsGrid.innerHTML = albumCardsContainer.innerHTML;
        
        // Show filter status if filtering is active
        const hasActiveFilters = filteredAlbums.length < totalCount;
        const filterStatus = document.getElementById('role-filter-status');
        if (filterStatus && hasActiveFilters) {
            filterStatus.style.display = 'block';
            const filterText = document.getElementById('current-role-filter');
            if (filterText) {
                filterText.textContent = `${filteredAlbums.length}/${totalCount} albums`;
            }
    ...
    calculateCollaboratorAlbumCount(artistName, collaboratorName) {
        const albumsGrid = document.getElementById('artist-albums-grid');
        if (!albumsGrid) return 0;

        const allAlbumsData = albumsGrid.getAttribute('data-all-albums');
        if (!allAlbumsData) return 0;
        ...
        const matchingAlbums = allAlbums.filter(album => {
            ...
            // Check if ALL collaborators in temp selection are in this album
            const albumCollaborators = album.credits
                .map(credit => credit.name)
                .filter(name => name !== artistName);
            
            return Array.from(tempSelection).every(selectedCollaborator =>
                albumCollaborators.includes(selectedCollaborator)
            );
        });

        return matchingAlbums.length;
    }
    ...
    // Show collaborator filter status
    showCollaboratorFilterStatus(collaborators, albumCount) {
        const filterStatus = document.getElementById('role-filter-status');
        ...
        if (filterStatus) {
            ...
            const albumText = albumCount === 1 ? '1 album' : `${albumCount} albums`;
            const artistName = document.getElementById('artist-albums-grid')?.getAttribute('data-artist') || '';
            const collaboratorText = collaborators.length === 1 
    ...
    // Clear all filters in artist modal (MULTI-SELECT)
    clearRoleFilter(artistName) {
        console.log(`🎭 Clearing all filters for ${artistName}`);

        // Clear multi-select state
        if (this.artistModalFilters && this.artistModalFilters.has(artistName)) {
            const filters = this.artistModalFilters.get(artistName);
        ...
        // Clear the filter UI state
        if (filterStatus) {
            filterStatus.style.display = 'none';
        }

        // Remove active filter styling from all tags
        ...
        document.querySelectorAll('.clickable-collaborator-filter').forEach(collaborator => {
    ...
    async generateRolesFromAlbumsAsync() {
        ...
        try {
            // Process albums in smaller batches to avoid blocking the UI
                                ...
                                roleNames.forEach(roleName => {
                                    if (roleName && typeof roleName === 'string') {
                                        ...
                                        if (cleanRoleName) {
                                            ...
                                            const artistId = credit.id || `artist-${credit.name.toLowerCase().replace(/\s+/g, '-').replace(/[^\w-]/g, '')}`;
                                            ...
                                            if (roleMap.has(cleanRoleName)) {
                                                ...
                                                if (existingArtist) {
                                                    ...
                                                    if (!existingArtist.albumIds.has(album.id)) {
                                                        existingArtist.albumIds.add(album.id);
                                                        existingArtist.albums.push(albumData);
                                                    }
                                                } else {
        ...
        // Convert map to array, clean up temporary data, and sort by frequency
        const rolesArray = Array.from(roleMap.values()).map(role => {
            // Clean up temporary albumIds Sets from artists
            role.artists.forEach(artist => {
                delete artist.albumIds;
            });
            return role;
    ...
    setupModalEventListeners() {
        ...
        this.handleModalClick = (event) => {
            ...
            // Handle role filter clicks in artist albums modal (MULTI-SELECT)
            ...
            // Handle genre filter clicks in artist albums modal (MULTI-SELECT)
            ...
            // Handle collaborator filter clicks in artist albums modal (multi-select)
            if (event.target.classList.contains('clickable-collaborator-filter')) {
                const collaborator = event.target.getAttribute('data-collaborator');
                const artistName = event.target.getAttribute('data-artist');
                if (collaborator && artistName) {
                    this.toggleCollaboratorFilter(artistName, collaborator, event.target);
                }
                return;
            }
